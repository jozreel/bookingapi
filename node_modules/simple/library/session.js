...
session.prototype.addtosessionlist = function (sesid, expdate) {
	...
	var obj = {};
	try {
		fstat = fs.statSync(require('path').normalize(cfg.tmppath + '/sessions/sessionlist.json'));
	}
	catch (err) {
	...
	try {
		if (fstat !== undefined) {
			var data = fs.readFileSync(cfg.tmppath + '/sessions/sessionlist.json');
			if(data.length == 0)
			  data="{}"
		...
		obj[sesid] = expdate;
		fs.writeFileSync(cfg.tmppath + '/sessions/sessionlist.json', JSON.stringify(obj));
	}
	catch (err) {
		console.log(err);
	}
...
session.prototype.removeFromSessionList = function(sessid)
{
...
session.prototype.end = function () {
	...
	var fs = require('fs');
	fs.unlink(require('path').normalize(require('../../../config/systemconfig').tmppath + '/sessions/' + this.decrypt(this.sessionreqvars.sessionid) + '.spf'), function (err) {
		if (err)
			console.log(err);
	...
	if (this.req.headers.cookie) {
		...
		for (var x in ckArray) {
			...
			if (ns[0] === 'sessionid') {
				...
				var cfg = require('../../../config/systemconfig');
				var fs = require('fs');
				if (fs.existsSync(path.normalize(cfg.tmppath + '/sessions/' + sessid + '.spf'))) {
