...
urlencode.prototype.checkanddecode =function(str)
{
...
urlencode.prototype.removeTags = function(string)
{
  let str =  string;
  if(string)
     str = string.replace(/<[^>]*>/g, ' ').replace(/\s{2,}/g, ' ').trim();
  return str;
}

urlencode.prototype.hasHtml = function(str)
{
  if(str)
   return (/<[^>]*>/g.test(str) || /\s{2,}/g.test(str));
  else return '';
}

urlencode.prototype.removeTagsAndStrip = function(string)
{
  if(string)
     return this.addspecial(this.removeTags(string));
  else 
     return '';
}
